...
# Agregar botón para iniciar la recolección
recoleccion_button = pygame_gui.elements.UIButton(
    relative_rect=pygame.Rect((10, 290), (150, 30)),
    text='Iniciar Recolección',
    manager=manager
)

# Variables para la recolección
empresa_origen = (50, 50)  # Empresa de origen
empresa_destino = (550, 550)  # Empresa de destino
material_seleccionado = None
cantidad_material = 0
estado_recoleccion = "esperando"  # "esperando", "recolectando", "entregando", "completado"
...
material_seleccionado = "carbón"

# Modificar la función recolectar_material para mostrar el mensaje al completar la recolección
def recolectar_material():
    global estado_recoleccion, mensaje_emergente
    if estado_recoleccion == "recolectando":
        # Mover hacia la empresa de origen
        if mover_carrito_a_punto(*empresa_origen, 2):
            estado_recoleccion = "entregando"
    elif estado_recoleccion == "entregando":
        # Mover hacia la empresa de destino
        if mover_carrito_a_punto(*empresa_destino, 2):
            estado_recoleccion = "completado"
            # Mostrar ventana emergente con el material transportado
            mensaje_emergente = pygame_gui.elements.UIWindow(
                pygame.Rect((150, 150), (300, 200)),
                manager=manager,
                window_display_title='Recolección Completada',
                object_id='#mensaje_emergente'
            )
            # Texto del mensaje emergente
            texto_mensaje = f"M. transportado: 8 unidades de {material_seleccionado}."
            pygame_gui.elements.UILabel(
                relative_rect=pygame.Rect((20, 10), (260, 150)),  # Ajusta el tamaño si es necesario
                text=texto_mensaje,
                manager=manager,
                container=mensaje_emergente
            )



# Dibuja las empresas de origen y destino
# Variables para la recolección
#posiciones de las empresas
empresa_origen = (50, HEIGHT - 50)  # Empresa de origen en la esquina inferior izquierda
empresa_destino = (WIDTH - 50, 50)  # Empresa de destino en la esquina superior derecha

def dibujar_empresas():
    # Solo dibujar si la recolección ha comenzado
    if estado_recoleccion == "recolectando" or estado_recoleccion == "entregando":
        pygame.draw.rect(window, yellow, (empresa_origen[0] - 20, empresa_origen[1] - 20, 40, 40))  # Empresa de origen
        pygame.draw.rect(window, yellow, (empresa_destino[0] - 20, empresa_destino[1] - 20, 40, 40))  # Empresa de destino
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT:
            ...
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED and event.ui_element == reset_button:
                ...
                estado_recoleccion = "esperando"  # Reiniciar estado de recolección
            ...
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED and event.ui_element == obstaculos_button:
                generar_obstaculos(2, 20)  # Generar 5 obstáculos de 40x40 píxeles

            if event.user_type == pygame_gui.UI_BUTTON_PRESSED and event.ui_element == recoleccion_button:
                estado_recoleccion = "recolectando"  # Iniciar recolección
    ...
    if estado_recoleccion == "recolectando" or estado_recoleccion == "entregando":
        recolectar_material()  # Manejar la recolección de materiales
    ...
    dibujar_empresas()  # Dibujar empresas en la pantalla
